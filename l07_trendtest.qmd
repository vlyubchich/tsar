...
**Sieve bootstrap** generally works for time series that are realizations of linear AR($\infty$) processes [@Buhlmann:2002], with more recent research extending to all stationary purely nondeterministic processes [@Kreiss:etal:2011].
...
We introduce these bootstrapped residuals $\epsilon^*_t$ back into the AR($\hat{p}$) model:
$$
X_t^* = \sum_{j=1}^{\hat{p}} \hat{\phi}_j X^*_{t-j} + \epsilon^*_t,
$$
where $\hat{\phi}_j$ are the estimated AR coefficients and $\hat{p}$ is the selected AR order.
