...
We do not know the distributional law under which sample means are distributed in this case, but we can approximate it using bootstrap.
...
The resampling step will give us artificial 'new' samples, while the statistics calculated multiple times will let us approximate the distribution of the statistic of interest.
Thus, by repeating the resampling and estimation steps multiple times, we will know how the statistic is distributed, even if the sample is small and not normally distributed.
...
1. Repeat the steps above a large number of times to obtain a distribution of the statistics $\bar{x}^*_1, \dots, \bar{x}^*_B$, where $B$ is the number of bootstrap replications. 
This is so-called *bootstrapped distribution*.
1. Finally, we use the bootstrapped distribution for the intended analysis. 
Typically it involves calculating confidence intervals. 
...
Note that the package `boot` provides functions with more options.
:::
...
In short, the first step of the outlined bootstrapping algorithm should preserve the dependence structure of the time series so that the generated bootstrapped samples correspond to the violation case that we want to accommodate in our analysis.
For example, if we want to accommodate serial dependence in the $t$-test, the bootstrapped time series should be still autocorrelated, so we can approximate the distribution of the test statistic when the assumption of independence is violated.

**Block bootstrap** text


**Sieve bootstrap** text
...
## Bootstrapped tests for trend detection
