...
with(D,
     ccf(AMO, Landings_noNA_sqrt, las = 1)
)
```
...
logconstant <- 1
with(Insurance,
     ccf(Precipitation, log(Claims + logconstant), las = 1, lag.max = 3)
)
```
...
The $k$ parameters represent the location parameter $\mu$ (that's what we typically model with regression models), scale $\sigma$, and two shape parameters: skewness and kurtosis.
The model can be further generalized for $k>4$, if needed.
It allows us fitting $k$ individual models to study relationships between regressors $\boldsymbol{x}$ and parameters of the response distribution:
$$
...
The terms in the GAMLSS provide a flexible framework to specify random effects and correlation structure as in the mixed effects models [@Zuur:etal:2009]; see Table 3 by @Stasinopoulos:Rigby:2007 for other possible specifications of the additive terms. 
Hence, models of the form @eq-gamlss may be a good choice for insurance problems, because such models accommodate non-normal distributions, possibly nonlinear relationships, and spatiotemporal dependencies in the data.

:::{.callout-note icon=false}

## Example: Insurance claims GAMLSS

Start by using the insights from the latest GARMA model to start with the selected regressors and distribution family of the response variable.

* Negative binomial distribution has two parameters (see `?NBI`) -- location and scale -- so the GAMLSS using this distribution family can include up to two equations.
We'll start by 
* 
...
library(gamlss)
set.seed(12345)
m03_gamlss <- gamlss(Claims ~ pb(Precipitation) + Week + Precipitation_lag1
                    ,sigma.formula = ~pb(Week) + pb(Precipitation_lag1)
                    ,correlation = corARMA(q = 3)
                    ,family = NBI
                    ,data = Insurance_noNA)
m03_gamlss$mu.coefSmo[[1]]$modelStruct$corStruct

m03_gamlss <- gamlss(Claims ~ re(fixed = ~ Precipitation + Week + Precipitation_lag1
                                 ,random = ~1|Year
                    ,correlation = corARMA(q = 3))
                    ,sigma.formula = ~pb(Week) + pb(Precipitation_lag1)
                    ,family = NBI
...
m01_gamlss <- gamlss(Claims ~ Precipitation + Week + Precipitation_lag1
                    ,sigma.formula = ~pb(Week) + pb(Precipitation_lag1)
                    ,correlation = corARMA(q = 10)
                    ,family = NBI
                    ,data = Insurance_noNA)
summary(m03_gamlss)
str(m03_gamlss)

m03_gamlss$mu.coefficients
m03_gamlss[[2]]
    [[1]]$coefficients$random.

AIC(m03nbi, m03_gamlss, m01_gamlss)
```
...
par(mfrow = c(1, 3))
term.plot(m1_gamlss, las = 1, se = TRUE, ylim = "common", what = "mu")

term.plot(m1_gamlss, las = 1, se = TRUE, ylim = "common", what = "sigma")

plot(m01_gamlss, ts = TRUE)

acf(residuals(m1_gamlss, what = "z-scores"))
acfResid(m1_gamlss)
AIC(m1_gamlss)
```
