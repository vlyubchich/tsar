...
However, when $n$ is small, there are not so many pairs $(X_{t+k}, Y_t)$ to calculate the correlations at large lags, hence the confidence bands should be ideally adjusted by using the smaller $n'(k) = n - |k|$ to account for this lack of confidence.
...
Intuitively, say if both $X_t$ and $Y_t$ are positively autocorrelated -- high (or low) values in $X_t$ follow each other, and high (or low) values in $Y_t$ follow each other -- there could be a time alignment among our considered lags $k = 0, \pm 1, \pm 2, \dots$ when the ups (or downs) in $X_{t+k}$ match those in $Y_t$. 
