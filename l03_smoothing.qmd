...
s1 <- bs(t, knots = tau, degree = 1)
s2 <- bs(t, knots = tau, degree = 2)
s3 <- bs(t, knots = tau, degree = 3)
...
Instead of fitting a global nonlinear function like in polynomial regresison (@sec-regressionTime) or fitting simpler local polynomials directly like in lowess smoothing (@sec-lowess), we can constuct a basis from basic splines (B-splines).
B-splines are simple functions, which vanish outside the intervals defined by timepoints called the 'knots.'
These splines are defined recursively, from the lowest to higher orders, starting from a simple indicator function for the intervals between the knots [see Appendix to Chapter 5 in @Hastie:etal:2009 for the recursive formulas]. 
Here we show results of the recursive computations, for times $t = 1, \dots,$ `r max(t)` and unequally-spaced knots `r paste0(tau, collapse = ", ")` (this is a more general case than the uniformly-spaced knots giving us cardinal B-splines).
...
\mu_t = a_0 + f(t),
$${#eq-GAM}
where $a_0$ is the intercept, and $f(t)$ represents the added effects of the $M$ basis functions $B_m(t)$ (hence the word 'additive' in GAM):
$$
f(t) = \sum_{m = 1}^M \beta_m B_m(t).
$$
We are not interested in the individual regression coefficients $\beta_m$ (regression parameters) we estimated, hence the function $f(t)$ is called *nonparametric* even though many parameters have been estimated to obtain $f(t)$ (similar to lowess smoothing).
...
#| label: fig-BSreg
#| echo: false
#| fig-cap: "Results of regressing `WWWusage` on the linear, quadratic, and cubic B-spline bases. The fit in each case is suboptimal because the knots were set arbitrarly."
...
The number and locations of the knots are important tuning parameters for achieving a desired smoothness.
...
The R packages `mgcv` and `gamlss` provide sevaral types of splines.
...
Here we use B-splines again (see `?mgcv::smooth.terms` and `?mgcv::b.spline` for different options), but allow the parameters to be estimated each time using a cross-validation (see `?mgcv::gam` for more details).

In the code below, we estimate the model four times, with different spline orders `m[1]` (quadratic or cubic) and basis dimensions `k` (it controls the overall smothness as it sets the upper limit on the degrees of freedom associated with a smooth, see `?mgcv::choose.k`).
