...
$$
(1 - B)X_t = X_t - X_{t-1} = Y_t
$$
give us back the uncorrelated series $Y_t$, hence $X_t$ is an ARIMA(0,1,0) process.
...
\end{split}
$$ {#eq-sarima}
where $D$ is the order of seasonal differences, $\Phi_1, \dots, \Phi_P$ are seasonal autoregression coefficients, $P$ is the seasonal autoregressive order, $\Theta_1, \dots, \Theta_q$ are seasonal moving average coefficients, $Q$ is the seasonal moving average order, and the rest of the terms are the same as in @eq-arima.
