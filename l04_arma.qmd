...
Check the same for random walk $S_t$. 
$S_t = \sum_{i=1}^t{X_t}$, where $X_t \sim \text{i.i.d.}(0,\sigma^2)$.
...
Notice the linear decay in ACF of nonstationary time series (@fig-rw).
...
::: {.callout-note}
Additionally, see that the ACF of this process
$$
...
$$
because, by the causality of the process, $Y_t - \hat{Y}_t$, depends only on $\{ \epsilon_{t+h-1}, \epsilon_{t+h-2},\dots \}$.
...
The representation of a causal stationary process $X_{t}$ with zero mean in the form
$$
...
Plugging different values of $h$ into the above equation we obtain
$$
\begin{split}
...
It adds a difference-based estimator by @Hall:VanKeilegom:2003, which is now used by default, and also switches to BIC as the model selection criterion. 
