...
1. Describe the mechanics of Fourier transformation for time series.
1. Present results of spectrum calculations using a periodogram.
...
#| fig-cap: "Monthly `AirPassengers` time series, log-transformed and detrended, and corresponding fast Fourier transformations."
#| fig-height: 7
...
## Example: NH4 concentration in wastewater system

For example, consider a time series `imputeTS::tsNH4` of NH4 concentration in a wastewater system (@fig-NH4). 
This time series is regularly spaced (every 10 minutes) but some of the observations are missing. 

```{r}
#| label: fig-NH4
#| fig-cap: "Time series of NH4 concentration in a wastewater system."

forecast::autoplot(imputeTS::tsNH4) +
    xlab("Day") +
    ylab("NH4 concentration") +
    theme_light()
```
...
```{r}
#| label: fig-NH4lombf
#| fig-cap: "Lomb--Scargle periodograms computed for the unequally-spaced time series of NH4 concentration."
